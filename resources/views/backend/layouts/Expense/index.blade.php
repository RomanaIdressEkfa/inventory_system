  ...
  <table class="table table-bordered rounded table-sm">
    <thead >
      <tr >
        ...
        <th class="bg-dark bg-gradient text-white">Category</th>
        <th class="bg-dark bg-gradient text-white">Amount</th>
        <th class="bg-dark bg-gradient text-white">Expense Date</th>
        <th class="bg-dark bg-gradient text-white">Notes</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{$expense->notes}}</td>
            <td>{{$expense->amount}}</td>
            <td>
                <a href="{{route('expense_details_view',$expense->id)}}" class="btn btn-info btn-sm text-white">View</a>
                <a href="{{route('expense_details_edit',$expense->id)}}" class="btn btn-primary btn-sm text-white">Edit</a>
                <a href="{{route('expense_details_delete',$expense->id)}}" onclick="return confirm('Are you sure to delete?')" class="btn btn-danger btn-sm text-white">Delete</a>
...
<a href="{{route('expense_details_create')}}" class="btn btn-primary mb-3 ">Add Expense</a>
