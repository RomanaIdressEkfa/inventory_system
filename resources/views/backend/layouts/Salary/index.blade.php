...
<div class="container border border-light shadow p-4 rounded">
    <h3 class=" text-primary mb-4 text-center" style='font-weight:bold'>ALL SALARY LIST</h3>
  <table class="table table-bordered rounded table-sm">
    <thead >

      <tr>
        <th class="bg-dark bg-gradient text-white">ID</th>
        <th class="bg-dark bg-gradient text-white">Month</th>
        <th class="bg-dark bg-gradient text-white">Year</th>
        <th class="bg-dark bg-gradient text-white">Salary Amount</th>
        {{-- <th class="bg-dark bg-gradient text-white">Tax Deductions</th>
        <th class="bg-dark bg-gradient text-white">Bonus</th> --}}
        <th class="bg-dark bg-gradient text-white">Employee Email</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{route('salary_details_view',$salary->id)}}" class="btn btn-info btn-sm text-white">View</a>
                <a href="{{route('salary_details_edit',$salary->id)}}" class="btn btn-primary btn-sm text-white">Edit</a>
                <a href="{{route('salary_details_delete',$salary->id)}}" onclick="return confirm('Are you sure to delete?')" class="btn btn-danger btn-sm text-white">Delete</a>
...
<a href="{{route('salary_details_create')}}" class="btn btn-primary mb-3 ">Add Category</a>
